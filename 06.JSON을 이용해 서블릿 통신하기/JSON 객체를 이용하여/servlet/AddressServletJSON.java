...
public class AddressServletJSON extends HttpServlet{
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        try{
            ...
            ResultSet result = state.executeQuery("select hakbun, name, age, tel, jumin, address from address");

            // ArrayList에 DB에서 받아온 데이터 값 JSON 파일로 만들어서 저장
            ArrayList<Person> persons = new ArrayList<>();
            while(result.next()){
                System.out.println(result.getString(6) + " " +result.getString(1) + " " +result.getString(2) + " " +result.getInt(3) + " " +result.getString(4) + " " +result.getString(5) + " ");
				Person p = new Person();
